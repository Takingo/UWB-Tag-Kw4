...
#define MAX_NUMBER_NIQ_CONNECTIONS (20)
 ...
 */
int niq_init(void (*start_uwb)(fira_device_configure_t * config, void * user_ctx), 
             void (*stop_uwb)(uint32_t session_id, void * user_ctx), 
 ...
 * @parm  conn_id - connection identifier, must be between 0 and MAX_NUMBER_NIQ_CONNECTIONS-1
 *        user_ctx - user pointer passed to stop_uwb()
 * 
 * @return 0 on success or
 *        -EINPVAL - incorrect input data
 */
int niq_stop_uwb(uint16_t conn_id, void * user_ctx);

/* @brief Provides Accessory Configuration Data for Out Of Band handshake.
 * @parm  conn_id - connection identifier, must be between 0 and MAX_NUMBER_NIQ_CONNECTIONS-1
 *        ranging_role - 0: Controlee+Responder, 1: Controller+Initiator
 *        uwbConfigData - buffer to populate UWB config data according to section 4.4.2 of Ultra Wideband Interoperability Specification for Nearby Interactions Release R1.pdf
 *        data_len - length of uwbConfigData
 * 
 * @return 0 on success or
 *        -EINPVAL - incorrect input data
 */
int niq_populate_accessory_uwb_config_data(uint16_t conn_id,
 ...
 * @parm  conn_id - connection identifier, must be between 0 and MAX_NUMBER_NIQ_CONNECTIONS-1
 *        buf - input data from NI service
 *        buf_len - length of data from NI service
 *        config - pointer to the fira device configuration
 *        user_ctx - user pointer passed to start_uwb()
 * 
 * @return 0 on success or
 *        -EINPVAL - incorrect input data
 *        -EVERSIONNOTSUPPORTED - version of NI protocol not supported
 */
int niq_configure_and_start_uwb(uint16_t conn_id,
                                uint8_t * buf, 
                                uint8_t buf_len, 
                                fira_device_configure_t * config,
                                void * user_ctx);
