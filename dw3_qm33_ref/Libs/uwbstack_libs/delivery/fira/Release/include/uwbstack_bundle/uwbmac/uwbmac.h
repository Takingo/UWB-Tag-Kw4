...
/**
 * uwbmac_calibration_transaction_start() - Start a calibration transaction allowing to update
 * configuration value(s).
 * @context: UWB MAC context.
 *
 * Setting calibration value(s) as part of a transaction allows to optimize the Flash usage.
 * When using a transaction, the new calibration values will be stored in the Flash only once, at
 * transaction ended. While when not using a transaction will cause as many Flash write as the
 * number of calibration values changed.
 * A calibration transaction should be performed by the following API call sequence:
 *  1. Start the transaction by calling ``uwbmac_calibration_transaction_start``.
 *  2. Update the calibration values by calling ``uwbmac_set_calibration`` for each of them.
 *  3. End the transaction by calling ``uwbmac_calibration_transaction_end``.
 *
 * Warning: starting a transaction without ending it will prevent storing values in the Flash, thus
 * causing a mismatch between persistent stored calibration, and the one used by the stack.
 * Values updated will be lost at next reboot.
 *
 * Return: QERR_SUCCESS or error.
 */
enum qerr uwbmac_calibration_transaction_start(struct uwbmac_context *context);

/**
 * uwbmac_calibration_transaction_end() - End the ongoing calibration transaction.
 * @context: UWB MAC context.
 *
 * That API should always be called after starting a transaction. See
 * ``uwbmac_calibration_transaction_start`` for more details.
 *
 * Return: QERR_SUCCESS or error.
 */
enum qerr uwbmac_calibration_transaction_end(struct uwbmac_context *context);
 ...
 *
 * Note: setting calibration value(s) as part of a transaction allows to optimize the Flash usage.
 * See ``uwbmac_calibration_transaction_start`` for more details.
 *
 * Return: QERR_SUCCESS or error.
 */
