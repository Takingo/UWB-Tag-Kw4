 ...
 * @file      ble_qnis.c
 *
 * @brief     QNIS (Qorvo NI Service) implementation.
 *
...
#define QNIS_BASE_UUID                                                                                     \
    {                                                                                                      \
        {                                                                                                  \
            0x02, 0x00, 0x12, 0xAC, 0x42, 0x02, 0xEB, 0xA1, 0xED, 0x11, 0x61, 0x6A, 0x00, 0x00, 0x93, 0x2E \
        }                                                                                                  \
...
static void on_connect(ble_qnis_t *p_qnis, ble_evt_t const *p_ble_evt)
{
    ...
    err_code = blcm_link_ctx_get(p_qnis->p_link_ctx_storage, p_ble_evt->evt.gap_evt.conn_handle, (void *)&p_client);
    ...
    err_code = sd_ble_gatts_value_get(p_ble_evt->evt.gap_evt.conn_handle, p_qnis->tx_handles.cccd_handle, &gatts_val);

    if ((err_code == NRF_SUCCESS) && (p_qnis->data_handler != NULL) && ble_srv_is_notification_enabled(gatts_val.p_value))
    {
        if (p_client != NULL)
        {
...
static void on_write(ble_qnis_t *p_qnis, ble_evt_t const *p_ble_evt)
{
    ...
    err_code = blcm_link_ctx_get(p_qnis->p_link_ctx_storage, p_ble_evt->evt.gatts_evt.conn_handle, (void *)&p_client);
    if (err_code != NRF_SUCCESS)
    {
    ...
    if ((p_evt_write->handle == p_qnis->tx_handles.cccd_handle) && (p_evt_write->len == 2))
    {
        if (p_client != NULL)
        {
    ...
    else if ((p_evt_write->handle == p_qnis->rx_handles.value_handle) && (p_qnis->data_handler != NULL))
    {
...
uint32_t ble_qnis_init(ble_qnis_t *p_qnis, ble_qnis_init_t const *p_qnis_init)
{
    ...
    /* Add the service. */
    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_qnis->service_handle);
    /**@snippet [Adding proprietary Service to the SoftDevice] */
    VERIFY_SUCCESS(err_code);
...
uint32_t ble_qnis_data_send(ble_qnis_t *p_qnis, uint8_t *p_data, uint16_t *p_length, uint16_t conn_handle)
{
    ret_code_t err_code;
