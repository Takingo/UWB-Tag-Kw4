...
#define ANIS_BASE_UUID                                                                                     \
    {                                                                                                      \
        {                                                                                                  \
            0xBA, 0x2D, 0x9C, 0x68, 0x73, 0x30, 0x33, 0x86, 0xB2, 0x4B, 0x17, 0x08, 0x00, 0x00, 0xFE, 0x48 \
        }                                                                                                  \
    } /**< Used vendor specific UUID. */
#define ACD_CHAR_BASE_UUID                                                                                 \
    {                                                                                                      \
        {                                                                                                  \
            0x28, 0x33, 0xF5, 0x75, 0x73, 0x80, 0x4E, 0x9A, 0x21, 0x47, 0xEF, 0xD8, 0x00, 0x00, 0xE8, 0x95 \
        }                                                                                                  \
    } /**< Used vendor specific UUID. */
...
static void on_connect(ble_anis_t *p_anis, ble_evt_t const *p_ble_evt)
{
    ...
    err_code = blcm_link_ctx_get(p_anis->p_link_ctx_storage, p_ble_evt->evt.gap_evt.conn_handle, (void *)&p_client);
    if (err_code != NRF_SUCCESS)
    {
        QLOGE("Link context for 0x%02X connection handle could not be fetched.", p_ble_evt->evt.gap_evt.conn_handle);
    }
    ...
    err_code = sd_ble_gatts_value_get(p_ble_evt->evt.gap_evt.conn_handle, p_anis->tx_handles.cccd_handle, &gatts_val);

    if ((err_code == NRF_SUCCESS) && (p_anis->data_handler != NULL) && ble_srv_is_notification_enabled(gatts_val.p_value))
    {
        if (p_client != NULL)
        {
...
static void on_write(ble_anis_t *p_anis, ble_evt_t const *p_ble_evt)
{
    ...
    err_code = blcm_link_ctx_get(p_anis->p_link_ctx_storage, p_ble_evt->evt.gatts_evt.conn_handle, (void *)&p_client);
    if (err_code != NRF_SUCCESS)
    {
        QLOGE("Link context for 0x%02X connection handle could not be fetched.", p_ble_evt->evt.gatts_evt.conn_handle);
    }
    ...
    if ((p_evt_write->handle == p_anis->tx_handles.cccd_handle) && (p_evt_write->len == 2))
    {
        if (p_client != NULL)
        {
    ...
    else if ((p_evt_write->handle == p_anis->rx_handles.value_handle) && (p_anis->data_handler != NULL))
    {
...
uint32_t ble_anis_init(ble_anis_t *p_anis)
{
    ...
    niq_populate_accessory_uwb_config_data(0, ACCESSORY_RANGING_ROLE, accessory_config_data_buffer, &accessory_config_data_length);
    ...
    /* Add the service. */
    err_code = sd_ble_gatts_service_add(BLE_GATTS_SRVC_TYPE_PRIMARY, &ble_uuid, &p_anis->service_handle);
    /**@snippet [Adding proprietary Service to the SoftDevice] */
    VERIFY_SUCCESS(err_code);
...
void set_accessory_uwb_config_data(uint8_t *curr_config)
{
    ...
    {
        QLOGE("Buffer allocation too small for Accessory Config Data. \n\r%d bytes allocated, %d bytes needed.", sizeof(accessory_config_data_buffer), accessory_config_data_length);
        return;
    }
